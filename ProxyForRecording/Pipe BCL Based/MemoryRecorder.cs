...
namespace IziHardGames.Proxy.Recoreder.MemoryBased
{
    ...
    public class MemoryRecorder
    {
        ...
        public virtual void Push(HttpSource dataSource, ReadOnlySequence<byte> data)
        {
            var existed = records.GetOrAdd(dataSource.id, (x) => RegistNewSource(dataSource));
            existed.Write(data);
        }
        public MemoryRecoreder RegistNewSource(HttpSource dataSource)
        {
    ...
    public class MemoryRecoreder : IDisposable
    {
        ...
        internal void Bind(HttpSource dataSource)
        {
