    ...
    public ref struct TlsHelloFromClientExtensionsEnumerator
    {
        ...
        public TlsHelloFromClientExtensionsEnumerator(in ReadOnlySequence<byte> data) : this()
        {
            lock (locker)
            {
                ...
                if (protocolVersion != ConstantsTls.CLIENT_VERSION12) throw new ArgumentOutOfRangeException($"Expected TLS1.2 But recived: {protocolVersion}");
#endif
                if (data.IsSingleSegment)
                {
                    TlsHelloFromClient.Read<IndexerForReadOnlySequence<byte>>(new IndexerForReadOnlySequence<byte>(data));
                ...
                else
                {
                    ...
                    var span = data.FirstSpan;
                    if (span.Length < 5) throw new System.NotImplementedException();
                    ushort msgSize = BufferReader.ToUshort(span[3], span[4]);
