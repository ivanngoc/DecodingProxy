...
namespace IziHardGames.Libs.Cryptography.Tls.Shared
{
    public ref struct TlsHelloFromClientExtensionsEnumerator
    {
        ...
        [HandshakeAnalyz]
        public TlsHelloFromClientExtensionsEnumerator(in HandshakeRecord clientHello, in ReadOnlySequence<byte> data) : this()
        {
            this.data = data;
            ushort msgSize = clientHello.record.Length;
            if (data.Length != msgSize) throw new ArgumentException($"Size must match");

            if (data.IsSingleSegment)
            {
                var span = data.FirstSpan;

                byte sessionIdLength = span[34];
                offset = 35;
            ...
            else
            {
                ...
                unsafe
                {
                    ...
                    byte sessionIdLength = data.GetItemAt(34);
                    offset = 35;
                    offset += sessionIdLength;
