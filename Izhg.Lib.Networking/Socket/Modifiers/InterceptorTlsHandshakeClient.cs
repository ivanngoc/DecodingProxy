...
namespace IziHardGames.Libs.IO
{
    public class InterceptorTlsHandshakeClient : SocketReaderInterceptor, IPoolBind<InterceptorTlsHandshakeClient>
    {
        ...
        public readonly TlsHandshakeReader tlsReader = new TlsHandshakeReader();
        ...
        public async Task AwaitFrame()
        {
            if (source is SocketReaderBuffered<SocketBufferDefault> reader)
            {
                ...
                while (true)
                {
                    ...
                    if (TlsHandshakeReader.CheckIntegrity(in mem))
                    {
                        break;
                    }
                }
        ...
        public void AnalyzFrame()
        {
            if (source is SocketReaderBuffered<SocketBufferDefault> reader)
            {
                ...
                if (clientVersion == ConstantsTls.CLIENT_VERSION_TLS13) protocolsTls = SslProtocols.Tls13;
                else if (clientVersion == ConstantsTls.CLIENT_VERSION_TLS12) protocolsTls = SslProtocols.Tls12;
                else if (clientVersion == ConstantsTls.CLIENT_VERSION_TLS11) protocolsTls = SslProtocols.Tls11;
