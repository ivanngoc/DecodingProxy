...
namespace IziHardGames.Libs.Networking.Servers
{
    public abstract class ServerBase<TClient>
        where TClient : IKey<uint>, IPoolBind<TClient>, IGuid, IDisposable, IGetLogger
    {
        ...
        {
            IPerfTracker tracker = client.Logger as IPerfTracker ?? throw new NullReferenceException();
            ...
            var remove = Task.Run(async () =>
            {
                ...
                tracker.ReportTime($"AcceptClientAsync finished task with status:{run.Status} exception:{run.Exception?.Message ?? "OK"}");
                ...
                tracker.ReportTime($"AcceptClientAsync Disposed");
            });
