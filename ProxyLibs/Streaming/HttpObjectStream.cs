...
using IziHardGames.Libs.ForHttp.Streaming;
...
namespace IziHardGames.Proxy.Sniffing.ForHttp
{
    ...
    public class HttpObjectStream : ReusableStream
    {
        ...
        {
            int end = indexStart + length;
            Span<char> hex = stackalloc char[HttpLibConstants.MAX_INT_CHARS];
            int count = default;
        ...
        private void InterpretNextFieldLine(int indexLineEnd)
        {
            ...
            {
                Query.From(new Wrap(header, 0, positionHeadersEnd)).Header(HttpLibConstants.FieldSubstrings.FIELD_TRANSFER_ENCODING).Values.Last().Is(HttpLibConstants.FieldValues.VALUE_CHUNKED);

                if (header.RangeContains(indexLineStart, currentLineLength, HttpLibConstants.FieldSubstrings.FIELD_TRANSFER_ENCODING))
                {

                }
                else if (header.RangeContains(indexLineStart, currentLineLength, HttpLibConstants.FieldSubstrings.SUBSTRING_CONTENT_LENGTH))
                {
        ...
        private void MapHeaders()
        {
            ...
            foreach (var field in fields)
            {
                ...
                switch (fieldName)
                {
                    case HttpLibConstants.FieldSubstrings.FIELD_TRANSFER_ENCODING:
                        {
                            if (field.Values.Last().Is(HttpLibConstants.FieldValues.VALUE_CHUNKED))
                            {

                            }
                            break;
                        }
                    case HttpLibConstants.FieldSubstrings.SUBSTRING_CONTENT_LENGTH:
                        {
                            break;
                        }
        ...
        public static int GetContentLength(StringBuilder sb, int start, int length)
        {
            ...
            Span<char> span = stackalloc char[HttpLibConstants.MAX_INT_CHARS];
            int indexStart = sb.IndexAfter(start, length, HttpLibConstants.FieldSubstrings.SUBSTRING_CONTENT_LENGTH);

            if (indexStart < 0) throw new ArgumentOutOfRangeException($"Substring {HttpLibConstants.FieldSubstrings.SUBSTRING_CONTENT_LENGTH} Not Founded in range start:{start} length:{length}");
